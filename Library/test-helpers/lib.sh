...
=head2 trusteeStartKbsContainer

Starts the KBS server in a podman container.

    trusteeStartKbsContainer [CONTAINER_NAME] [IMAGE] [CONFIG_FILE]

=over

=item CONTAINER_NAME (optional)

The name for the container. Defaults to kbs-trustee-test.

=item IMAGE (optional)

The container image to use. Defaults to ghcr.io/confidential-containers/staged-images/kbs:latest.
Callers may need to pin a specific version to match the RCAR protocol version
used by the installed trustee-guest-components RPM.

=item CONFIG_FILE (optional)

The path to the KBS configuration file. Defaults to $(pwd)/config/kbs-config.toml.
Must be an absolute path since it is bind-mounted into the container.

=back

This function is used instead of trusteeStartKbsServer when the KBS binary
on the host lacks the coco-as-builtin feature (e.g., the RPM-packaged trustee-kbs).
The container image is built with coco-as-builtin support, enabling RCAR attestation.

The function mounts the following directories into the container (matching
paths used by trusteeCreateKbsConfig):

=over

=item * Config file

=item * KBS repository (/opt/confidential-containers/kbs/repository)

=item * KBS policy (/opa/confidential-containers/kbs)

=item * AS work directory (/opt/confidential-containers/attestation-service)

=item * HTTPS certificates (if HTTP_MODE is set to https)

=back

Note: The RCAR protocol version must match exactly between client and server
(strict =X.Y.Z check). If the installed trustee-guest-components RPM uses a
different protocol version than :latest, pass a pinned image as the IMAGE argument.

The function will rlDie if the container fails to start.

=cut

trusteeStartKbsContainer() {
    local container_name=${1:-"kbs-trustee-test"}
    local image=${2:-"ghcr.io/confidential-containers/staged-images/kbs:latest"}
    local config_file=${3:-"$(pwd)/config/kbs-config.toml"}

    # Paths must match those in the KBS config generated by trusteeCreateKbsConfig
    local kbs_repo_dir="/opt/confidential-containers/kbs/repository"
    local kbs_policy_dir="/opa/confidential-containers/kbs"
    local as_work_dir="/opt/confidential-containers/attestation-service"

    if [[ ! -f "${config_file}" ]]; then
        rlFail "KBS config file not found at '${config_file}'. Run trusteeCreateKbsConfig first."
        return 1
    fi

    rlRun "podman pull ${image}" 0 "Pull KBS container image"

    # Build volume mount list
    local podman_volumes="-v ${config_file}:${config_file}:Z"
    ...
    rlRun "podman run -d --replace --name ${container_name} --network host \
        ${podman_volumes} \
        ${image} \
        /usr/local/bin/kbs --config-file ${config_file}" \
...
=head2 trusteeStopKbsContainer

Stops and removes a KBS container started by trusteeStartKbsContainer.

    trusteeStopKbsContainer [CONTAINER_NAME]

=over

=item CONTAINER_NAME (optional)

The name of the container to stop and remove. Defaults to kbs-trustee-test.

=back

This function stops the container and then force-removes it.
It is safe to call even if the container is not running.

=cut

trusteeStopKbsContainer() {
    local container_name=${1:-"kbs-trustee-test"}

    rlRun "podman stop ${container_name} 2>/dev/null || true" 0,1 "Stop KBS container"
    rlRun "podman rm -f ${container_name} 2>/dev/null || true" 0,1 "Remove KBS container"
}
